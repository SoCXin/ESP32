/*
   This file about bt task to sub player
	by:qitas
*/
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    if (eir) {
        ...
        if (strcmp((char *)peer_bdname, BT_REV) != 0) {
            return;
        }
    ...
    case ESP_BT_GAP_DISC_STATE_CHANGED_EVT: {
        if (param->disc_st_chg.state == ESP_BT_GAP_DISCOVERY_STOPPED) {
            if (m_a2d_state == APP_AV_STATE_DISCOVERED) {
                ...
                ESP_LOGI(TAG, "a2dp connecting to peer: %s", peer_bdname);
...
static void bt_app_av_sm_hdlr(uint16_t event, void *param)
{
    ESP_LOGI(TAG, "%s state %d, evt 0x%x", __func__, m_a2d_state, event);
    switch (m_a2d_state) {
    ...
    case APP_AV_STATE_DISCONNECTING:
        ...
        break;
    default:
        ESP_LOGE(TAG, "%s invalid state %d", __func__, m_a2d_state);
        break;
    }
}
/**
...
static void bt_app_av_state_unconnected(uint16_t event, void *param)
{
    ...
    case BT_APP_HEART_BEAT_EVT: {
        uint8_t *p = peer_bda;
        ESP_LOGI(TAG, "a2dp connecting to peer: %02x:%02x:%02x:%02x:%02x:%02x",
                 p[0], p[1], p[2], p[3], p[4], p[5]);
        esp_a2d_source_connect(peer_bda);
        m_a2d_state = APP_AV_STATE_CONNECTING;
        m_connecting_intv = 0;
    ...
    case APP_AV_MEDIA_STATE_STOPPING: {
        if (event == ESP_A2D_MEDIA_CTRL_ACK_EVT) {
            ...
            if (a2d->media_ctrl_stat.cmd == ESP_A2D_MEDIA_CTRL_STOP &&
                    a2d->media_ctrl_stat.status == ESP_A2D_MEDIA_CTRL_ACK_SUCCESS) {
                ESP_LOGI(TAG, "a2dp media stopped successfully, disconnecting...");
                m_media_state = APP_AV_MEDIA_STATE_IDLE;
